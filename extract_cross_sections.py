...
def produce_absorption_spectra(molecule):
    """
    The purpose of this code is to produce absorption spectra of a given molecule across all wavelengths for which it is available.

    In order to coincide with our goal to match the test cases provided by the work of Rugheimer and collaborators, the wavelength range of these data must span at least 130-250 nm.

    input:
    -molecule

    Output:
    -writes file of cross-sections.
    ...
    if molecule=='co2':
        ...
        For wavelengths from 201.6 nm to 300 nm, we use the work of Shemansky (1972). Their discussion implies pretty clearly that their cross-sections include both scattering and absorption. In particular, they argue that the extinction coefficient above 204 nm is due entirely to Rayleigh scattering.
        """
    ...
    plt.savefig('./Plots/xc_'+molecule+'.eps', orientation='portrait',papertype='letter', format='eps')
    plt.show()
    """
...
def produce_rayleigh_scattering(mol):
    """
